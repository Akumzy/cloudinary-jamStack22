...
export default function ChatRoom({ user }: { user: User }) {
  ...
  const channelCreator = channelMembers
    ...
    : null
  const isChannelMember = channelMembers?.some((member: any) => member.userId === user?.userId)
  ...
  function handleJoinChannel() {
    ...
    socket?.emit("join_channel", { channelId: id, userId: user.userId }, (error: any, channel: any) => {
      if (error) {
